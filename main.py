...
__domains = ["api22-core-c-useast1a.tiktokv.com","api21-core-c-useast1a.tiktokv.com","api19-core-c-useast1a.tiktokv.com","api16-core-c-useast1a.tiktokv.com"]
        ...
        try:
            ...
            try:
                if response.json()['status_code'] == 0:
                    _lock.acquire()
                    print(Colorate.Horizontal(Colors.purple_to_red, f'TikTok Viewbot by Weon ^| success: {success} fails: {fails} reqs: {reqs} rps: {rps} rpm: {rpm}'))
                    success += 1
                    _lock.release()
            except:
...
def fetch_proxies():
    url_list =["https://raw.githubusercontent.com/jetkai/proxy-list/main/online-proxies/txt/proxies.txt",
...
if __name__ == "__main__":
    ...
    try:
        link = str(Write.Input("\n", Colors.white_to_green, interval=0.0001))
        __aweme_id = str(
        ...
        os.system("cls" if os.name == "nt" else "clear")
        input(Col.red + "NHẬP ID SỐ" + Col.reset)
        sys.exit(0)
